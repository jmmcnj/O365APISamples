...
namespace O365APIweb
{
    //Code for update to preview
    static class SitesApiSample
    {
        ...
        public static async Task<bool> Uploaddoc(HttpPostedFileBase hpf)
        {
            var client = await EnsureClientCreated();
            var isSuccess = false;
            IFile filesResults = null;
            try
            {
                var fileNameArr = hpf.FileName.Split('\\');
                var fileNameNoPath = fileNameArr[fileNameArr.Count() - 1];
                filesResults = await client.Files.AddAsync(fileNameNoPath, true, hpf.InputStream);
                isSuccess = true;
            }
